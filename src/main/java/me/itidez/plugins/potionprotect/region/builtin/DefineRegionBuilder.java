	...
	public DefineRegionBuilder(Player p, Location loc1, Location loc2, String regionName, String creator) {
		...
		if (regionName.equals("")){
			for(int i = 0; true; i++){
				...
				if (Potionprotect.rm.getRegion(regionName, p.getWorld()) == null){
		...
		for(int xl = minX; xl<=maxX; xl++) {
			if (Potionprotect.rm.regionExists(xl, minZ, p.getWorld())||Potionprotect.rm.regionExists(xl, maxZ, p.getWorld())){
				setError(p, RED + "You're overlapping another region.");
				return;
			}
		}
		for(int zl = minZ; zl<=maxZ; zl++) {
			if (Potionprotect.rm.regionExists(minX, zl, p.getWorld())||Potionprotect.rm.regionExists(maxX, zl, p.getWorld())){
				setError(p, RED + "You're overlapping another region.");
				return;
			}
		}
		...
		if (Potionprotect.rm.isSurroundingRegion(r, p.getWorld())){
