	...
	public EncompassRegionBuilder(SignChangeEvent e) {
		...
		if (regionName.equals("")){
			for(int i = 0; true; i++){
				...
				if (Potionprotect.rm.getRegion(regionName, w) == null){
		...
		for (int i = 0; i < Potionprotect.maxScan; i++){
			int nearbyCount = 0;
			int x = current.getX();
			int y = current.getY();
			int z = current.getZ();
			...
			if((Potionprotect.blockID == 55)){
			...
			}else if((Potionprotect.blockID == 85)||(Potionprotect.blockID == 101)||(Potionprotect.blockID == 113)){
				block = new Block[6];
				blockSize = 6;
				block[0] = w.getBlockAt(x+1, y, z);
				block[1] = w.getBlockAt(x-1, y, z);
				...
				//Potionprotect.logger.warning("The block ID you have chosen isn't valid!");
				block = new Block[6];
				blockSize = 6;
				block[0] = w.getBlockAt(x+1, y, z);
				block[1] = w.getBlockAt(x-1, y, z);
			...
			for (int bi = 0; bi < blockSize; bi++){
				...
				if ((Potionprotect.blockID == 85)&&((block[bi].getType().equals(Material.FENCE))||(block[bi].getType().equals(Material.FENCE_GATE)))){
					validBlock = true;
				}else{
					validBlock = (block[bi].getTypeId() == Potionprotect.blockID);
				}
				...
				if (i != 0) {
					...
					if (current.equals(first)) {
						...
						for (Block ib : redstone){
							other = Potionprotect.rm.getRegion(ib.getLocation());
							if (other != null){
						...
						if (Potionprotect.rm.isSurroundingRegion(region, w)){
							setErrorSign(e, "You're completely surrounding another region.");
							return;
						}
						int pLimit = Potionprotect.ph.getPlayerLimit(p);
						boolean areaUnlimited = Potionprotect.ph.hasPerm(p, "Potionprotect.unlimited");
						int totalArea = Potionprotect.rm.getTotalRegionSize(p.getName());
						if (pLimit >= 0){
