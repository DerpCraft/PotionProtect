...
public class Region implements Serializable {
	...
	/*public boolean rename(String newName){
		World w = Potionprotect.rm.getWorld(this);
		if (Potionprotect.rm.getRegion(newName, w) != null) return false;
		name = newName;
		return true;
	}*/
	...
	public Region(String name, LinkedList<String> owners, int[] x, int[] z){
		...
		if (size == 4){
			//Potionprotect.logger.debug("One of these regions, is not like the other ones, one of these regions, is an mbr.");
			this.x = null;
			this.z = null;
		}
		this.owners = owners;
		...
		//Potionprotect.logger.debug("Bounding rect: " + maxMbrX + ", " + minMbrX + ", " + maxMbrZ + ", " + minMbrZ);
	}

	/*public void addToRM(World w){
		Potionprotect.rm.add(this, w);
	}*/

	public void delete(){
		Potionprotect.rm.remove(this);
	}
	...
	/*public boolean intersects(int bx, int bz, String w){
		String world = Potionprotect.rm.getWorld(this).getName();
	...
	public boolean intersects(int bx, int bz) {
		if (this.x == null) {
			//Potionprotect.logger.info("x = null. =(");
			return true;
		}
		int i, j;
		boolean ret = false;
	...
	public boolean canBuild(Player p){
		if(p.getLocation().getY() < Potionprotect.heightStart) return true; //For mining and stuff.
		return (isOwner(p) || isMember(p) || Potionprotect.ph.hasPerm(p, "Potionprotect.bypass"));
	}
	...
	public boolean canPVP(Player p){
		...
		return (Potionprotect.ph.hasPerm(p, "Potionprotect.bypass"));
	}
