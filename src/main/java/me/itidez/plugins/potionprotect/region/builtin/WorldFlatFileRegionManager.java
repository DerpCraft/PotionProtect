...
public class WorldFlatFileRegionManager implements WorldRegionManager {
	...
	public void save(){
		try {
			Potionprotect.debug("RegionManager.Save(): File type is " + Potionprotect.fileType.toString());
			String world = getWorld().getName();
			File datf = new File(Potionprotect.pathData, "data_" + world + ".regions");
			if(!datf.exists()){
				datf.createNewFile();
			}
	...
	private void backupRegions(File datf){
		if(!Potionprotect.backup)
			return;
		File dataBackup = new File(Potionprotect.pathData, "data_" + getWorld().getName() + ".regions.backup");
		dataBackup.delete();
	...
	public void load(){
		String world = getWorld().getName();
		load(Potionprotect.pathData + "data_" + world + ".regions");
	}

	@SuppressWarnings("unchecked")
	private void load(String path){
		String world = getWorld().getName();
		String datbackf = (Potionprotect.pathData + "data_" + world + ".regions.backup");
		File f;
		if(!((f = new File(path)).exists())) {
			try {
				f.createNewFile();
				Potionprotect.log("Created new world region file: (" + path + ").");
			} catch (IOException e) {
				e.printStackTrace();
			}
		}
		try {
			if (!Util.isFileEmpty(path)){
				...
				switch(Potionprotect.fileType) {
				case yml:
					Potionprotect.debug("RegionManager.load() File type: yml");
					Yaml yml = new Yaml();
					Object oyml;
					...
					break;
				case ymlgz:
					Potionprotect.debug("RegionManager.load() File type: ymlgz");
					Yaml ymlgz = new Yaml();
					Object oymlgz;
				...
				case oos:
					Potionprotect.debug("RegionManager.load() File type: oos");
					if(ois == null) {
				...
				if(Potionprotect.backup && backupExists() && !path.equalsIgnoreCase(datbackf)){
					load(datbackf);
					Potionprotect.log("Main data file is blank, Reading from backup.");
					return;
				}
				Potionprotect.log("Creating a new data file.");
			...
			if(Potionprotect.backup && backupExists() && !path.equalsIgnoreCase(datbackf)){
				load(datbackf);
				Potionprotect.log("The data file is corrupt. Loading from backup.");
				return;
			}
		} catch (IOException e) {
			e.printStackTrace();
